...
namespace VolumeTransitioner
{
    namespace Audio
    {
        // Written by Sverrir Sigmundarson (https://github.com/sverrirs)
        // Code retrieved from https://gist.github.com/sverrirs/d099b34b7f72bb4fb386
        // Modifications include simplifications for nullable and inlined 'out' variables
    ...
    {
        public static void SavePreset(string path, int max, int min, bool onMax)
        {
            try
            {
                File.WriteAllText(path, $"MaxVol={max}\nMinVol={min}\nIsSetToMaxVol={onMax}");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"There was an error saving the current preset: {ex.Message}", "Error saving preset", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
